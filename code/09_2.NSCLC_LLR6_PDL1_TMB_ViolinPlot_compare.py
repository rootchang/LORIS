...
#Description: Comparison LLR6 vs. PDL1 vs. TMB scores between responders and non-responders on training and test sets
#             (Fig. 6c)
#Run command: python 09_2.NSCLC_LLR6_PDL1_TMB_ViolinPlot_compare.py
###############################################################################################
...
if __name__ == "__main__":
    ...
    for sheet_i in range(start_set,5):
        ...
        y_true = np.array(data['y'].tolist())
        temp_df = pd.DataFrame()
        temp_df['PDL1_score'] = np.log2(y_pred_PDL1[y_true == 1]+1)
        plot_data_PDL1.append(y_pred_PDL1[y_true == 1])
        plot_PDL1_df = pd.concat([plot_PDL1_df, temp_df], axis=0)
        temp_df = pd.DataFrame()
        temp_df['PDL1_score'] = np.log2(y_pred_PDL1[y_true == 0]+1)
        plot_data_PDL1.append(y_pred_PDL1[y_true == 0])
        plot_PDL1_df = pd.concat([plot_PDL1_df, temp_df], axis=0)
    plot_PDL1_df = plot_PDL1_df.reset_index(drop=True)
    ...
    fig, axes = plt.subplots(1, 3, figsize=(6.3, 1.7))
    ...
    ###################################### axes[1]: PDL1 ######################################
    graph = sns.violinplot(y="PDL1_score", x='Dataset', hue="Response", data=plot_df,
                           palette=my_palette, linewidth=0.5, saturation=0.75,
