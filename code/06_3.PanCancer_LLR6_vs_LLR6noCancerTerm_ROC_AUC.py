###############################################################################################
#Aim: Pan-cancer LLR6 vs. LLR6NC comparison
#Description: AUC comparison between original LLR6 vs. LLR6 without using the cancer type calibration term (LLR6NC)
#             on training and multiple test sets (Extended Data Fig. 9b).
...
if __name__ == "__main__":
    ...
    featuresNA_LLR5 = ['TMB', 'Systemic_therapy_history', 'Albumin', 'NLR', 'Age', 'CancerType1',
                      'CancerType2', 'CancerType3', 'CancerType4', 'CancerType5', 'CancerType6', 'CancerType7',
                      'CancerType8', 'CancerType9', 'CancerType10', 'CancerType11', 'CancerType12', 'CancerType13',
                      'CancerType14', 'CancerType15', 'CancerType16']
    ...
    for c in x_test_LLR5_list:
        x_test_scaled_list.append(pd.DataFrame(scaler_sd.transform(c)))
    clf = linear_model.LogisticRegression().fit(x_test_scaled_list[0], y_test_list[0])
    clf.coef_ = np.array([params_dict['LLR_coef'][0:5] + [0 for _ in range(16)]]) # set all cancer type calibration terms to 0
    clf.intercept_ = np.array(params_dict['LLR_intercept'])
    for i in range(len(x_test_scaled_list)):
        y_pred_test = clf.predict_proba(x_test_scaled_list[i])[:, 1]
    ...
    ############################## Plot ROC curves ##############################
    textSize = 8
    output_fig1 = '../03.Results/PanCancer_LLR6_LLR6noCancerTerm_ROC_AUC_compare.pdf'
    ax1 = [0] * 6
    ...
    for i in range(6):
        ...
        if not i:
            ax1[i].plot(fpr, tpr, color=palette[1], linestyle='-',
                        label='LLR6NC AUC: %.2f (%.2f, %.2f)' % (AUC, AUC_05, AUC_95))
